class Task_page < SitePrism::Page
	elements :save_button, '#SAVE' # Botão salvar da tarefa
	element :subject_field, "input[id='name']" # Assunto da tarefa
	element :status_select, :xpath, "//select[@id = 'status']" # Status tarefa
	element :priority_select, :xpath, "//select[@id = 'priority']" #Prioridade da tarefa
	element :subject_task, "span[id='name']"
	element :status_task, "input[id='status']"
	element :priority_task, "input[id='priority']"
	element :view_task_button, "div[id='actionMenuSidebar']" 
	elements :editar_links, "a[class='edit-link']"
